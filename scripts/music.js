const handleMusic = async () => {
    const res = await fetch("https://openapi.programming-hero.com/api/phero-tube/category/1001");
    const data = await res.json();
    displayMusicVideos(data.category);

}

const displayMusicVideos = (videos) => {
    // 1. Hold the container
    const musicVideosConatainer = document.getElementById('music-videos-container')
...
<div class="flex gap-4 pt-5 px-0">
    ...
    `
        musicVideosConatainer.appendChild(div)
    });
}
handleMusic();