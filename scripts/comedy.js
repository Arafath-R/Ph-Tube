const handleComedy = async () => {
    const res = await fetch("https://openapi.programming-hero.com/api/phero-tube/category/1003");
    const data = await res.json();
    displayComedyVideos(data.category);

}

const displayComedyVideos = (videos) => {
    // 1. Hold the container
    const comedyVideosConatainer = document.getElementById('comedy-videos-container')
...
<div class="flex gap-4 pt-5 px-0">
    ...
    `
        comedyVideosConatainer.appendChild(div)
    });
}
handleComedy();